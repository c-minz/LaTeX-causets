...
It is also possible to use multiple causets, for example in a partial order of causet ``growth''.
Here the causets appear as text in the TikZ node contents (within the \lstinline&{}&), so we need to use the \lstinline&\pcauset& (\lstinline&\rcauset& or \lstinline&\causet&) commands without the \lstinline&draw& prefix.
\lstinputlisting{causets_example2.tex}
\begin{center}
  \includegraphics[scale=0.80]{causets_example2}
  (rescaled to 80\%)
\end{center}
WARNING: Causet diagrams use TikZ nodes for the elements and element labels, so that changing how nodes are drawn in TikZ (either globally or in a local scope around a \lstinline&causets& macro) will also affect the nodes of elements and element labels.
Here we use multiple layers to avoid inheriting the style from the parent node for all \lstinline&\pcauset& macros.
The first layer consists of the big circular nodes with ``poscau node'' style and the second layer has the \lstinline&\pcauset& macros as content of TikZ node without style.
...
% \subsection[v1.6, unreleased (Crown)]{v1.6, unreleased}
% 
% \subsubsection{New features}
% 
% \begin{itemize}\itemsep=-1ex
% \item \versioncauset{v1-5-1}{\rcauset{1,5,3,7,2,6/{orange},4}{3/6}}%
%   \dots
% \end{itemize}


\subsection[v1.5.1, unreleased (Butterfly)]{v1.5.1, unreleased}
...
\item \versioncauset{v1-5}{\pcauset{1,5,3,6/{XMLv1.5, label=XML, labels={XMLv1.5, above=1ex, at=north}},2,4}}%
  Improved examples and added warning on nesting causet diagrams within TikZ nodes.
\end{itemize}


\subsection[v1.5, 2025-02-24 (Butterfly)]{v1.5, 2025-02-24}

\subsubsection{New features}

\begin{itemize}\itemsep=-1ex
\item Added support for \LaTeXML\ including the key \lstinline&alt={&\PH\lstinline&}& to define an alternative text representing a diagram for screen readers.
