...
For example: 
\begin{lstlisting}
\causetsSetSizes{1.50ex}{0.15ex}{0.06ex}{1.30ex}{0.35ex}{1.20}
...
Similarly, all events of a causet with $N$ events are the nodes \lstinline§(Ei)§, the event labels are the nodes are \lstinline§(ELi)§, the $u$-axis labels are \lstinline§(EULi)§, and the $v$-axis labels are \lstinline§(EVLi)§, where $\smash{i \in [ 1, N ]}$. 
