...
\causet{...}{..,i-j,..}
\end{lstlisting}
The first command takes a permutation of the integers $[ 1, N ]$ (where $N$ is the cardinality of the causet) that determines the event positions and causal structure such that events $i$ and $j$ are linked if $i < j$ and there is no $k > j$ between $i$ and $j$. 
...
\begin{lstlisting}
\causetsSetSizes{0.45ex}{0.01ex}{0.25ex}{0.08ex}{0.27} % small
\causetsSetSizes{0.90ex}{0.02ex}{0.50ex}{0.15ex}{0.50} % normal
\causetsSetSizes{1.80ex}{0.04ex}{1.00ex}{0.27ex}{1.00} % large
\end{lstlisting}
...
\begin{tikzpicture}
	...
	\node[draw=blue, inner sep=1pt, thick, ellipse, fit=(e4) (e6) (e7)] (Finf) {};
	\node[draw=blue] (FinfLabel) at (6.5, -0.5) {future infinity};
	\node[draw=red] (centerLabel) at (0.0, -0.5) {graph center};
	\node[draw=green!50!black] (myEventLabel) at (5.5, -2.5) {event (e2)};
	\draw[ultra thick, blue, ->] (FinfLabel) -- (Finf);
	\draw[ultra thick, red, ->] (centerLabel) -- (3, -1);
	\draw[ultra thick, green!50!black, ->] (myEventLabel) -- (e2);
