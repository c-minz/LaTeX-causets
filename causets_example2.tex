...
% Set styles:
\colorlet{future}{green!50!black}
\colorlet{past}{yellow!30!orange!75!white}
\colorlet{anywhere}{red!75!white}
\tikzset{
  prob arrow/.style={line width=1.5pt}
}
\tikzcausetsset{
  labels={gray},
  unlabeled  % comment this line to show labels
}
...
\begin{tikzpicture}[-stealth]
  ...
  \begin{scope}[poscau node/.style={draw, thin, circle, minimum size=1.1cm}]
    \node[poscau node] (C1) at (0, 0) {};
    \node at (C1) {\pcauset{1}};
    \node[poscau node] (C12) at (-4*\xstep, 1*\ystep) {};
    \node at (C12) {\pcauset{1,2}};
    \node[poscau node] (C21) at (4*\xstep, 1*\ystep) {};
    \node at (C21) {\pcauset{2,1}};
    \foreach \perm [count=\i from 0] in {123, 132, 312, 213, 321}{%
      \expandafter\defCevents\perm
      \node[poscau node] (C\perm) at (4*\i*\xstep-8*\xstep, 2*\ystep) {};
      \node at (C\perm) {\pcauset{\Ea,\Eb,\Ec}};
    }
    \foreach \perm [count=\i from 0] in {1234, 1243, 1423, 1324, 1432, 4123, 4132, 3124, 3412, 3142, 4312, 2134, 2143, 4213, 3214, 4321}{%
      ...
      \node[poscau node] (C\perm) at (2*\i*\xstep-15*\xstep, 3.75*\ystep) {};
      \node at (C\perm) {\pcauset{\Ea,\Eb,\Ec,\Ed}};
    }
  \end{scope}
  ...
  \begin{scope}[past]
    \foreach \perma/\permb in {12/213, 123/3124, 123/2134, 132/1243, 132/3142, 132/2143, 312/1423, 312/4213, 213/1324, 213/3124, 213/3214, 321/1432, 321/4132, 21/132}
      \drawprobarrow{C\perma}{}{C\permb};
  \end{scope}
  % ... to anywhere (additionally, further):
  \begin{scope}[anywhere]
    \drawprobarrow{C123}{}{C1324};
  \end{scope}
  % ... to the future:
  \begin{scope}[prob arrow, future]
  ...
  \end{scope}
  % Add legend:
  \node[above left, align=left, draw, inner sep=3ex]
    at (15.5*\xstep, -0.5*\xstep)
    {Adding elements\\
    \textcolor{future}{$\to$}
      in the future (green)\\
    \textcolor{past}{$\to$}
      also in the past (yellow)\\
    \textcolor{anywhere}{$\to$}
      or even anywhere (red)};
\end{tikzpicture}
\end{document}
